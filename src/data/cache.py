...
Saves DataFrames as parquet in data/raw/ with timestamps, so the model
can load cached data instantly and only hit APIs when refreshing.
"""
...
CACHE_DIR = Path(get("data_paths.raw", "data/raw"))


def _cache_path(component: str) -> Path:
    return CACHE_DIR / f"{component.lower().replace(' ', '_').replace('/', '_')}.parquet"
...
def clear(component: str | None = None) -> None:
    """Delete cached data for one or all components."""
    if component:
        path = _cache_path(component)
        if path.exists():
            path.unlink()
